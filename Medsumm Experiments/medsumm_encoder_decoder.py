...
from transformers.models.bart.modeling_bart import (
    ...
    BartEncoderLayer,
    shift_tokens_right,_expand_mask,_make_causal_mask, _expand_mask
...
SOURCE_COLUMN = 'Hinglish_Question'
TARGET_COLUMN = 'multimodal_Question_summ'
VISUAL_INPUT_PATH = 'resnet_embedding'
...
def preprocess_dataset(dataset):
    ...
    # print("resnet embedding type here : ", type(dataset["resnet_embedding"]))
    # print("resnet embedding shape here : ", dataset["resnet_embedding"].shape)
    # print("resnet embedding shape here : ", dataset["resnet_embedding"])
    # model_inputs['visual_input'] = torch.tensor(dataset["resnet_embedding"]).to(DEVICE)
    # tem = dataset["resnet_embedding"]
    # print("tem ty[e : ", type(tem))
    # print("tem ", tem)
    # print("tem 96 : ", len(tem.iloc[96]))
    # for i,j in enumerate(tem):
    #     print(i," : ",j, " : ", len(tem.iloc[j]))

    res_vec = []
    for i in dataset["resnet_embedding"]:
        # print(type(i)," : ", len(i))  
        res_vec.append(torch.tensor(i))
    # print(len(res_vec))      
    # print(type(res_vec[0]))
    # print(len(res_vec[0]))  
      
    res_vec = torch.stack(res_vec)   
    # print("res_vec shape : ", res_vec.shape)   
    # model_inputs['visual_input'] = dataset["resnet_embedding"].to(DEVICE)
    model_inputs['visual_input'] = res_vec.to(DEVICE)
    
    #model_inputs['visual_input'] = torch.tensor([v for v in visual['input_ids']], dtype=torch.long, device=DEVICE)
...
class ContextAwareAttention(nn.Module):
    ...
    def forward(self,
        ...
        print("Key context shape:",key_context.shape)
        print("K shape:",k.shape)
        print("self.w1_k(k) shape:",self.w1_k(k).shape)
        print("self.w2_k() shape:",self.w2_k(key_context).shape)
...
class ContextAwareAttentionDecoder(nn.Module):

    def __init__(self,
                 dim_model: int,
                 dim_context: int,
                 dropout_rate: Optional[float]=0.0):
        super(ContextAwareAttentionDecoder, self).__init__()
...
class MultimodalBartDecoder(BartPretrainedModel):
    """
    ...
    def forward(
        ...
        visual_input=None,  #Visual Input Added
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if idx in self.fusion_at_layer:
                print("Type_visual_inputs: ",type(visual_input))
                visual_input = self.visual_transformer(visual_input)[-1]
                print("Decoder:")
                print("Visual input",visual_input.shape)
...
class MultimodalBartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        ...
        self.decoder = MultimodalBartDecoder(config, self.shared)
    ...
    def forward(
        ...
        # If the user passed a tuple for encoder_outputs, we wrap it in a BaseModelOutput when return_dict=True
            ...
            print("Encoder output is present:")
            print("Type_image: ",type(visual_input))
...
class MAFdecoder(nn.Module):
    ...
    def __init__(self,
                 dim_model: int,
                 dropout_rate: int):
        super(MAFdecoder, self).__init__()
        self.dropout_rate = dropout_rate
        
        self.visual_context_transform = nn.Linear(VISUAL_MAX_LEN, TARGET_MAX_LEN, bias=False)


        self.visual_context_attention = ContextAwareAttentionDecoder(dim_model=dim_model,
                                                              dim_context=VISUAL_DIM,
                                                              dropout_rate=dropout_rate)   
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        #scores.append(test_results)
        path = "./Model/Model_TI_encoder_decoder_image_fusion_hinglish_light"
        save_model(model,path,tokenizer)
        print("Model saved at path: ", path)
        #df = pd.DataFrame(scores)
        #csv_file_path = 'scores_TI_encoder_decoder_image_fusion.csv'
...
#NewDataset_path = 'Dataset/Final_D.json'
#NewDataset_path = 'Final_D.json'  # add the file path
#Dataset = pd.DataFrame(read_json_data(NewDataset_path))
#Dataset=pd.read_json(NewDataset_path)
#print(Dataset.columns)

#hinglish=pd.read_csv('multimodal_hinglish_dialog_final_updated.csv')
#Dataset['Hinglish_dialog_wo_disease']=hinglish['Hinglish_dialog_wo_disease']

#data = Dataset['Img_features']#torch.load('resnet_image_vector.pt').tolist()

#print("Dataset type : ", type(Dataset))
# with open("resnet_embedding.pkl",'rb') as file:
#     data=pickle.load(file)
# print("resnet vector type : ", type(Dataset['resnet_vector'][0]))
# print("data type : ", type(data))
# print("data length : ", len(data))
# print("data 0 shape : ", type(data[0]))
# print("data : ", data)
#Dataset['resnet_embedding']=data   
#print(Dataset.keys())
	
#print(type(Dataset['Img_features'].values))


vgg_train = torch.load('vgg_vectors_medsumm_train.pt').tolist()
vgg_val= torch.load('vgg_vectors_medsumm_val.pt').tolist()
vgg_test=torch.load('vgg_vectors_medsumm_test.pt').tolist()

train_data=pd.read_csv("train_medsumm.csv")
test_data=pd.read_csv("test_medsumm.csv")
valid_data=pd.read_csv("val_medsumm.csv")

train_data['resnet_embedding']=vgg_train
test_data['resnet_embedding']= vgg_test
valid_data['resnet_embedding']=vgg_val


#data['Img_features'] = data['Img_features'].apply(ast.literal_eval)

#train_data['resnet_embedding'] =train_data['resnet_embedding'].apply(ast.literal_eval)
#test_data['resnet_embedding'] =test_data['resnet_embedding'].apply(ast.literal_eval)
#valid_data['resnet_embedding'] =valid_data['resnet_embedding'].apply(ast.literal_eval)
